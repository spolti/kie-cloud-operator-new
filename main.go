...
import (
	"flag"
	"fmt"
	"github.com/spolti/kie-cloud-operator-new/controllers/kieapp"
...
func init() {
	...
	utilruntime.Must(appv2.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "5b0d9380.kiegroup.org",
	})
	if err != nil {
	...
	if err = (&kieapp.KieAppReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		log.Error(err, "unable to create controller", "controller", "KieApp")
		os.Exit(1)
	}
	...
	if os.Getenv(constants.NameSpaceEnv) == "" {
		log.Warn("%s required env %s not set, please configure downward API", message, constants.NameSpaceEnv)
	}
	if os.Getenv(constants.OpNameEnv) == "" {
		log.Warn("%s required env %s not set, please configure env", message, constants.OpNameEnv)
	}
}

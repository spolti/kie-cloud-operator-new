...
func deployConsole(ctx context.Context, reconciler *KieAppReconciler, operator *appsv1.Deployment) {
	...
	if semver.Compare(reconciler.OcpVersion, "v4.2") >= 0 || reconciler.OcpVersion == "" {
		...
		if err := reconciler.Get(ctx, types.NamespacedName{Name: new.Name, Namespace: new.Namespace}, existing); err != nil {
			if errors.IsNotFound(err) {
				log.Info("Creating ConfigMap ", new.Name)
				if err := reconciler.Create(ctx, new); err != nil {
			...
			if !reflect.DeepEqual(existing.Labels, new.Labels) {
				...
				if err := reconciler.Update(ctx, existing); err != nil {
...
func loadCounterparts(reconciler *KieAppReconciler, namespace string, requestedMap map[reflect.Type][]client.Object) (map[reflect.Type][]client.Object, error) {
...
func updateCSVlinks(reconciler *KieAppReconciler, route *routev1.Route, operator *appsv1.Deployment) {
	var patch bool
	found := &routev1.Route{}
	for i := 1; i < 60; i++ {
		time.Sleep(time.Duration(100) * time.Millisecond)
		err := reconciler.Get(context.TODO(), types.NamespacedName{Name: route.Name, Namespace: route.Namespace}, found)
...
func patchCSVObject(reconciler *KieAppReconciler, cur, mod *operatorsv1alpha1.ClusterServiceVersion) error {
	patch, err := client.MergeFrom(cur).Data(mod)
	if err != nil || len(patch) == 0 || string(patch) == "{}" {
		return err
	}
	return reconciler.Patch(context.TODO(), cur, client.RawPatch(types.MergePatchType, patch))
}
